...
async function ensureDataStore() {
    ...
    if (!store.metadata) {
        store.metadata = { seededDefaults: false };
        updated = true;
    }
    if (!store.metadata?.seededDefaults) {
        const seedAssets = [
        ...
        if (assetsToInsert.length > 0) {
            const mappedAssets = assetsToInsert.map((asset, index) => ({
...
async function startServer() {
    ...
    app.put("/api/users/profile", async (req, res) => {
        ...
        const { name, email, preferredTheme, notificationsEnabled } = req.body;
        ...
        const store = await readStore();
        const emailConflict = store.users.some((entry) => entry.id !== user.id && entry.email.toLowerCase() === email.toLowerCase());
        if (emailConflict) {
    ...
    app.post("/api/assets", async (req, res) => {
        ...
        const { name, assetTag, category, status, holder } = req.body;
